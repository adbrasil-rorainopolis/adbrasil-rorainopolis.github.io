...
async function carregarMisArrecadacao(){
  ...
  const linhas = Object.values(linhasMap).sort((a, b) => SGEG.ordemCongregacaoIdxG(a.nome, a.conselho) - SGEG.ordemCongregacaoIdxG(b.nome, b.conselho));
...
async function _popularConselhosMis(idSel, valor){
  ...
  const { porConselho } = await SGEG.mapaConselhos();
  sel.innerHTML = '<option value="Todos">Todos os conselhos</option>' + SGEG.ordenarConselhosG(Object.keys(porConselho)).map(c => `<option value="${escM(c)}">${escM(c)}</option>`).join('');
...
async function _popularCongsMis(){
  ...
  const unicas = SGEG.ordenarCongregacoesG([...new Set(lista)]);
